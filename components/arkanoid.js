export class Arkanoid {
  ...
  createArk(flip, sprite) {
    this.arkanoid = this.relatedScene.physics.add
      .image(400, 550, sprite)
      .setImmovable()
      .setOrigin(0.5, 0.5);
  ...
  platformImpact(fireball, arkanoid) {
    let relativeImpact = fireball.x - arkanoid.x;
  ...
  arkMove(teclas, fireball, arkanoid) {
    if (teclas.left.isDown) {
      ...
      if (fireball.isGlued) {
        fireball.get().body.position.x =
          this.relatedScene.arkanoid.get().body.position.x + 55;
      }
    } else if (teclas.right.isDown) {
      this.arkanoid.setVelocityX(700);
      if (fireball.isGlued) {
        fireball.get().body.position.x =
          this.relatedScene.arkanoid.get().body.position.x + 75;
      }
    } else {
